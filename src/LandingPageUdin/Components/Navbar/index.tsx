...
const Navbar: React.FC<NavbarProps> = ({ currentPage, isMenuOpen, handleMenuClick, setIsMenuOpen }) => {
  ...
  const renderMenuItems = () =>
    MENU_ITEMS.map((item) => (
      <div
        key={item}
        className={`cursor-pointer text-base px-3 py-1 rounded-full transition-all duration-300 ${currentPage === item ? "text-[#144470] font-bold text-lg" : "text-black hover:text-[#144470]"}`}
